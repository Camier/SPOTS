...
class EnhancedCoordinateExtractor:
    ...
    def __init__(self):
        ...
        self.toulouse_bounds = {"min_lat": 42.5, "max_lat": 44.5, "min_lon": -1.0, "max_lon": 3.0}
    ...
    def _validate_coordinates(self, lat: float, lon: float) -> bool:
        ...
        return (
            self.toulouse_bounds["min_lat"] <= lat <= self.toulouse_bounds["max_lat"]
            and self.toulouse_bounds["min_lon"] <= lon <= self.toulouse_bounds["max_lon"]
        )
...
if __name__ == "__main__":
    ...
    for text in test_texts:
        ...
        if coords:
            logger.info(f"✓ Found: {coords} from '{text[:50]}...'")
        else:
            logger.info(f"✗ No coordinates in '{text[:50]}...'")
