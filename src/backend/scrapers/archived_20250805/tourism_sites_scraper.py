...
def convert_to_spot_format(tourism_spot):
    ...
    is_hidden = any(
        keyword in tourism_spot["description"].lower() for keyword in ["secret", "caché", "méconnu", "peu connu"]
    )
...
def main():
    logger.info("🏛️ Starting tourism sites scraper...")
    ...
    logger.info(f"   Hidden/secret: {hidden}")
