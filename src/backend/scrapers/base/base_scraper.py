...
try:
    ...
    from ..utils.rate_limiter import RateLimiter, ScraperRateLimiters
    from ..utils.session_manager import SessionManager

    SpotDataValidator = SpotValidator
    EnhancedValidator = SpotValidator
    real_data_validator = RealDataValidator()
    HAS_ENHANCED_MODULES = True
except ImportError:
    HAS_ENHANCED_MODULES = False
    SpotDataValidator = None
    EnhancedValidator = None
    real_data_validator = None
...
class BaseScraper(ABC):
    ...
    def save_spot(self, spot_data: Dict) -> bool:
        ...
        try:
            ...
            # Apply real data validation
            if real_data_validator and self.source_name:
                if not real_data_validator.validate_data_source(spot_data, self.source_name):
                    self.logger.error("Real data validation failed - blocking mock/fake data")
                    return False
        ...
        try:
            ...
            if self.session_manager:
                self.session_manager.save_session(
                    self.session, {"last_run": datetime.now().isoformat(), "spots_saved": saved}
                )
