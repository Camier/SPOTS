...
class SpotDataValidator:
    ...
    def __init__(self):
        ...
        self.spot_schema = Schema(
            {
                ...
                SchemaOptional("extracted_name"): And(str, lambda s: 3 <= len(s) <= 200),
                SchemaOptional("location_type"): And(str, lambda s: s in self.LOCATION_TYPES),
                SchemaOptional("activities"): And(str, Use(self._validate_activities)),
                SchemaOptional("is_hidden"): And(Use(int), lambda x: x in [0, 1]),
                SchemaOptional("metadata"): dict,
                # Additional validation fields
                SchemaOptional("confidence_score"): And(Or(float, Use(float)), lambda x: 0 <= x <= 1),
        ...
        self.coord_pattern = re.compile(r"(-?\d+\.?\d*)[,\s]+(-?\d+\.?\d*)")
        self.name_cleanup_pattern = re.compile(r"[\x00-\x1f\x7f-\x9f]")  # Control characters
    ...
    def _postprocess_data(self, data: Dict) -> Dict:
        ...
        if "location_type" not in enhanced:
            enhanced["location_type"] = self._infer_location_type(enhanced.get("raw_text", ""))
    ...
    def _extract_coordinates(self, text: str) -> Optional[Tuple[float, float]]:
        ...
        for match in matches:
            try:
                ...
                # Validate bounds
                if (
                    self.TOULOUSE_BOUNDS["lat_min"] <= lat <= self.TOULOUSE_BOUNDS["lat_max"]
                    and self.TOULOUSE_BOUNDS["lon_min"] <= lon <= self.TOULOUSE_BOUNDS["lon_max"]
                ):
                    return lat, lon
    ...
    def validate_batch(self, spots: List[Dict]) -> Tuple[List[Dict], List[Dict]]:
        ...
        for spot in spots:
            try:
                ...
                invalid.append({"data": spot, "error": str(e)})
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info(f"Confidence score: {validated['confidence_score']}")
        logger.info(f"Extracted coords: {validated.get('latitude')}, {validated.get('longitude')}")
        logger.info(f"Location type: {validated.get('location_type')}")
        logger.info(f"Tags: {validated.get('tags')}")
    except SchemaError as e:
