...
def query_overpass(query_template, bbox):
    ...
    try:
        ...
        logger.info(f"Error querying Overpass: {e}")
        return None
    ...
    for element in data["elements"]:
        ...
        if not name:
            if spot_type == "waterfalls":
                name = f"Cascade non nommée ({element['lat']:.4f}, {element['lon']:.4f})"
...
def main():
    logger.info("🗺️ Starting OpenStreetMap scraper...")
    logger.info(f"   Searching area: {BBOX}")
    ...
    for spot_type, query in OSM_QUERIES.items():
        ...
        if data:
            ...
            logger.info(f"   Found {len(spots)} {spot_type}")
    ...
    logger.info(f"   Unique spots: {len(unique_spots)} (removed {len(all_spots) - len(unique_spots)} duplicates)")
    ...
    # Summary
    logger.info(f"\n✅ Complete!")
    logger.info(f"   Total spots found: {len(all_spots)}")
    logger.info(f"   Unique spots: {len(unique_spots)}")
    logger.info(f"   New spots saved: {saved}")
