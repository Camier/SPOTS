    ...
    def execute_with_retry(self, func: Callable, *args, **kwargs) -> Optional[any]:
        ...
        for attempt in range(self.max_retries):
            ...
            try:
                ...
                is_rate_limit = any(
                    term in error_msg for term in ["rate limit", "too many requests", "429", "throttle"]
                )
...
class ScraperRateLimiters:
    ...
    def instagram():
        ...
        return RateLimiter(min_delay=2.0, max_delay=5.0, backoff_factor=2.5, max_retries=3, circuit_breaker_threshold=5)

    @staticmethod
    def reddit():
        """Reddit API has clear rate limits"""
        return RateLimiter(
            min_delay=1.0, max_delay=2.0, backoff_factor=2.0, max_retries=5, circuit_breaker_threshold=10
        )
    ...
    def osm():
        ...
        return RateLimiter(
            min_delay=0.5, max_delay=1.5, backoff_factor=1.5, max_retries=3, circuit_breaker_threshold=15
        )
