...
class SpotDataValidator:
        ...
        self.schema = Schema(
            {
                ...
                Optional("latitude"): Or(None, And(Use(float), lambda n: 42.5 <= n <= 44.5)),
                Optional("longitude"): Or(None, And(Use(float), lambda n: -1.0 <= n <= 3.0)),
                # Enum fields
                Optional("location_type"): Or(None, lambda s: s in self.LOCATION_TYPES),
                Optional("activities"): Or(None, str, self._validate_activities),
                # Boolean fields
                Optional("is_hidden"): Or(0, 1, bool),
                # Timestamp
                Optional("scraped_at"): Or(None, self._validate_timestamp),
            ...
            if isinstance(value, str):
                ...
                for pattern in self.sql_injection_patterns:
                    if re.search(pattern, value, re.IGNORECASE):
                        raise ValueError(f"Potential SQL injection in field '{key}': {value[:50]}...")
    ...
    def _validate_activities(self, activities: str) -> bool:
        ...
        return any(any(valid in activity.strip() for valid in self.ACTIVITIES) for activity in activity_list)
    ...
    def validate_batch(self, spots: list) -> tuple:
        ...
        for i, spot in enumerate(spots):
            try:
                ...
                invalid_spots.append({"index": i, "error": str(e), "spot": spot})
...
if __name__ == "__main__":
    ...
    logger.info("Testing valid spot:")
    try:
